...
{
    void save (tenncor::Source& out, ade::Tensor* in) override
    {
        ...
        out.set_label(static_cast<Variable*>(in)->label_);
    }
};
...
auto vec = arr.data();\
return get_variable(std::vector<TYPE>(vec.begin(), vec.end()), shape);

/// Unmarshal tenncor::Source as Variable containing context of source
struct DataLoader final : public pbm::iDataLoader
{
    ade::Tensorptr load (const tenncor::Source& source) override
    {
